...
#model_S = flows_model_RQS_old(device=device)
...
for try_ in range(1):
    ...
    for w_ in [0.006]:
        ...
        # sample_C = []
            ...
            #   print(shuffles)
                for model_path in list_of_files:
                    choice = np.random.choice(len(SR_data), size = 10_000, replace = True)
                    mass = SR_data[:,0][choice].reshape(-1,1)
                ...
                # mass_CR_samples = torch.tensor(kde_CR.sample(1000).reshape(-1,1)).float().to(device)
                    ...
                    with torch.no_grad():
                       ...
                       x_samples_SR = model_S.sample(1, mass_SR_samples)
                # x_samples_CR = model_S.sample(1, mass_CR_samples).detach().cpu().reshape(-1,4)
                    x_samples_SR = x_samples_SR.reshape(-1,4)
                   ...
                   # x_samples_SR = inverse_logit_transform(x_samples_SR, pre_parameters["min"], pre_parameters["max"])
                    ...
                    x_samples_SR = torch.hstack((mass_SR_samples, x_samples_SR))
                    x_samples_SR = x_samples_SR[~torch.isnan(x_samples_SR).any(axis=1)]
                    x_samples_SR = torch.hstack((x_samples_SR, torch.ones((x_samples_SR.shape[0],1))))
                    x_samples_SR = inverse_transform(x_samples_SR, pre_parameters)
          ...
          #  log_prob_B_SR = np.array(log_prob_B)
            x_samples_SR = np.concatenate(sample_S,axis=0)
        ...
        # x_samples_CR = np.concatenate(sample_C)
            ...
            np.save(f'./figures/samples_{nsig}.npy', x_samples_SR)
          #  np.save(f'./results/{wa}')
          #  print(log_prob_S_SR.shape)
          #  print(log_prob_B_SR.shape)
           # sample_dict['SR'][f'{w_}_{nsig}'] = x_samples_SR
           # sample_dict['SR'][f'{nsig}'] = x_samples_SR
