...
for epoch in range(args.epochs):
    trainloss=anode_uncond(model.model,trainloader,model.optimizer,pre_parameters ,device=device, mode='train')
    valloss=anode_uncond(model.model,valloader,model.optimizer,pre_parameters, device=device, mode='val')
...
x_samples_train = generate_transformed_samples_uncond(model.model, traintensor, pre_parameters, device=device).cpu().detach().numpy()
x_samples_val = generate_transformed_samples_uncond(model.model, valtensor, pre_parameters, device=device).cpu().detach().numpy()
...
for i in range(4):
    figure=plt.figure()
    #if dims > 1:
    plt.hist(all_data[:,i+1],bins=100, density=True, label=f'data for {i}', histtype='step')
