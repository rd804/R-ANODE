...
parser.add_argument('--CR_path', type=str, default='results/nflows_lhc_co/CR_uncond_1000/try_1_0', help='CR data path')
...
for i in best_epochs:
    ...
    log_B_ = model_B.model.log_probs(inputs=valtensor_B[:,1:-1])
...
for i in best_epochs:
    model_B = DensityEstimator(args.config_file, eval_mode=True, load_path=f"{args.CR_path}/model_CR_{i}.pt", device=device)
    log_B_ = model_B.model.log_probs(inputs=traintensor_B[:,1:-1])
...
train_tensor = torch.utils.data.TensorDataset(traintensor_S, log_B_train_tensor)
val_tensor = torch.utils.data.TensorDataset(valtensor_S, log_B_val_tensor)
...
model_S = flows_model_RQS(device=device, context_features=None)
...
epochs = args.epochs
optimizer = torch.optim.AdamW(model_S.parameters(),lr=1e-3)
...
for epoch in range(args.epochs):
    ...
    train_loss = r_anode_uncond(model_S,model_B.model,w_,optimizer ,trainloader, 
                         pre_parameters, device=device, mode='train',\
                          data_loss_expr=args.data_loss_expr)
    val_loss = r_anode_uncond(model_S,model_B.model,w_,optimizer, valloader, 
    ...
    if args.wandb:
        if epoch % 50 == 0:
            if ~(np.isnan(train_loss) or np.isnan(val_loss)):
                ...
                x_samples = model_S.sample(len(traintensor_S[:,0])).detach().cpu()
...
if not args.ensemble:
    ...
    for index in sorted_index:
        ...
        for i, data in enumerate(testloader):
                log_S_.extend(model_S.log_prob(data[:,1:-1]).cpu().detach().numpy().tolist())
...
#  model_B = DensityEstimator(args.config_file, eval_mode=True, load_path=f"{args.CR_path}/my_ANODE_model_epoch_{epoch}.par", device=device)
  ...
  #  with torch.no_grad():
    ...
    with torch.no_grad():
        log_p = evaluate_log_prob_uncond(model_B.model, testtensor_B, pre_parameters['CR'], 
