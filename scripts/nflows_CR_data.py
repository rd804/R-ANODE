...
if os.path.exists(f'{save_path}model_CR_best.pt'):
...
x_train = preprocess_params_transform(data, pre_parameters)
...
alltensor = torch.cat((traintensor, valtensor), dim=0).cpu().detach()
all_data = np.vstack((train_data, val_data))
x_samples = generate_transformed_samples(model.model, alltensor, pre_parameters, device=device).cpu().detach().numpy()
...
bins = [np.linspace(0.0,7.0,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50)]
for i in range(1,5,1):
    figure=plt.figure()
    #if dims > 1:
    plt.hist(all_data[:,i][all_data[:,-1]==0],bins=100, density=True, label=f'background', histtype='step')
    plt.hist(all_data[:,i],bins=100, density=True, label=f'data', histtype='step')
    plt.hist(x_samples[:,i-1],bins=100, density=True, label=f'sample', histtype='step')
    plt.title(f'NFlow vs data for {i}')
    plt.legend(loc='upper right')
    plt.savefig(f'{save_path}/data_{i}.png')
    if args.wandb:
        wandb.log({f'nflow_SR': wandb.Image(figure)})

    plt.close()

mask_SR = (alltensor[:,0]>3.3) & (alltensor[:,0]<3.7)

all_data = np.vstack((train_data, val_data))
mask_SR_numpy = (all_data[:,0]>3.3) & (all_data[:,0]<3.7)

x_samples = generate_transformed_samples(model.model, alltensor[mask_SR], pre_parameters, device=device).cpu().detach().numpy()

bins = [np.linspace(3.3,3.7,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50)]
for i in range(1,5,1):
    figure=plt.figure()
    #if dims > 1:
    plt.hist(all_data[mask_SR_numpy,i],bins=100, density=True, label=f'data', histtype='step')
    plt.hist(all_data[mask_SR_numpy,i][all_data[mask_SR_numpy,-1]==0],bins=100, density=True, label=f'background', histtype='step')
    plt.hist(x_samples[:,i-1],bins=100, density=True, label=f'samples', histtype='step')
    plt.title(f'NFlow vs data for {i}')
    plt.legend(loc='upper right')
    plt.savefig(f'{save_path}/SR_{i}.png')
    if args.wandb:
        wandb.log({f'nflow_SR': wandb.Image(figure)})
...
mask_CR = ~mask_SR
mask_CR_numpy = ~mask_SR_numpy

x_samples = generate_transformed_samples(model.model, alltensor[mask_CR], pre_parameters, device=device).cpu().detach().numpy()

bins = [np.linspace(0.0,7.0,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50), np.linspace(0,1,50)]
for i in range(1,5,1):
    figure=plt.figure()
    #if dims > 1:
    plt.hist(all_data[mask_CR,i],bins=100, density=True, label=f'data', histtype='step')
    plt.hist(all_data[mask_CR_numpy,i][all_data[mask_CR_numpy,-1]==0],bins=100, density=True, label=f'background', histtype='step')
    plt.hist(x_samples[:,i-1],bins=100, density=True, label=f'samples', histtype='step')
    plt.title(f'NFlow vs data for {i}')
    plt.legend(loc='upper right')
    plt.savefig(f'{save_path}/CR_{i}.png')
    if args.wandb:
