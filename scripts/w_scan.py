...
for k,w in enumerate(weights):
   ...
   # print('tprs_ranode shape', len(tprs_ranode))
    ...
    sic_fpr_median_ranode , sic_fpr_max_ranode, sic_fpr_min_ranode, fpr_cut_ranode = \
    ensembled_SIC_at_fpr(tprs_ranode, fprs_ranode)
...
for k,w in enumerate(weights):
   ...
   # print('tprs_ranode shape', len(tprs_ranode))
    ...
    # index_max = np.argmax(sic_median_ranode)
    index_max = 2
    _median_sic_fpr_RANODE.append(sic_fpr_median_ranode[index_max])
    _sic_fpr_min_RANODE.append(sic_fpr_min_ranode[index_max])
    _sic_fpr_max_RANODE.append(sic_fpr_max_ranode[index_max])
...
_median_sic_fpr_RANODE = np.array(_median_sic_fpr_RANODE)
_sic_fpr_min_RANODE = np.array(_sic_fpr_min_RANODE)
_sic_fpr_max_RANODE = np.array(_sic_fpr_max_RANODE)

weight_array = np.array([i if i != 'true_w' else true_w for i in weights])
mask = weight_array != true_w
print(weight_array)
plt.plot(weight_array, _median_sic_RANODE, marker=".", label='max SIC')
...
plt.plot(weight_array, _median_sic_fpr_RANODE, marker=".", label='SIC at FPR=1e-3')
plt.fill_between(weight_array[mask], _sic_fpr_min_RANODE[mask], _sic_fpr_max_RANODE[mask], alpha=0.3)
plt.axvline(x=true_w, linestyle='--', label=r'$w_{true}$', color='black')
#plt.err
plt.xlabel(r'$w$',fontsize=15)
plt.ylabel('SIC',fontsize=15)
plt.xticks(fontsize=15)
plt.yticks(fontsize=15)
plt.annotate(r'$N_{sig}$='+f'{nsig}', xy =(0.01, 4), xycoords='data',fontsize=15)
#plt.title(r'$N_{sig}$='+f'{nsig}',fontsize=15)
plt.xscale('log')
