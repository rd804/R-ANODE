...
do
   ...
   # for sig in 1
    for w_ in 0.001 0.002 0.004 0.005
    ...
    do
        ...
        for try_ in ${arr[@]}
            do
                ...
                --export=try_=${try_},group=${group},job_type=${job_type},sig=${w_} r_anode_resample_wscan_submission.sh ${try_} ${group} ${job_type} ${w_} &
                # get job id
            done
    done
